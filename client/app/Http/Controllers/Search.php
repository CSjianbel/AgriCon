...
class Search extends Controller
{
    public function search(Request $request) {
        ...
        //get all the dictionary that matches the search input based on names
        $searchResults = array();
        $jsonData = json_decode($jsonData, true);

        foreach($jsonData as $product) {
            if (strpos(strtolower($product['name']), strtolower($searchInput)) !== false) {
                array_push($searchResults, $product);
            }
        }

        // dd($searchResults);

        return view('home', ['products' => $searchResults]);
    }
}
